...
# Emma V. Sage - University of California, Berkeley

start_time <- Sys.time() # program start time

# User inputs ------------------------------------------------------------------
project <- 'myproject'
stilt_wd <- file.path('/home/ubuntu', project)
...
t_start <- 'YYYY-MM-DD 00:00:00'
t_end   <- 'YYYY-MM-DD 00:00:00'
run_times <- seq(from = as.POSIXct(t_start, tz = 'UTC'),
                 to   = as.POSIXct(t_end, tz = 'UTC'),
                 by   = '6 hours')

# Receptor location(s)
lati <- #add receptor latitude
long <- #add receptor longitude
zagl <- #add height of receptor (we used 25 for EPA monitors)
...
smooth_factor <- 1
time_integrate <- F
xmn <- -125 #footprint lat/long specifications - this covers all of CA
xmx <- -113
ymn <- 32
ymx <- 43
xres <- 0.03
